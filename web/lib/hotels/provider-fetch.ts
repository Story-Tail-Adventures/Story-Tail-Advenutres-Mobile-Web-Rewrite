 ...
 *
 * Bypassing the patch also means bypassing the Data Cache entirely, and that is a real cost
 * now rather than a free win. An earlier draft justified it by saying hotel search had its own
 * six-hour Postgres cache; it does not any more — that was dropped deliberately. So this
 * genuinely forgoes caching at our layer, and the reason is still sound: the Data Cache keys
 * on the request URL, and SerpApi puts the credential IN that URL, so caching here would
 * write the secret to disk on every node that serves a search. What softens the loss is the
 * provider's own cache — SerpApi serves identical parameter sets free for an hour and does
 * not count them against the monthly tier (Free-Travel-APIs.md:294-296), which is why
 * `providerParams` in query.ts is treated as an interface rather than an implementation
 * detail.
 *
